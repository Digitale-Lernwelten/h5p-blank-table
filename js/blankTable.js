/*global H5P*/
H5P.BlankTable = (function ($, Question) {
  /**
   * @constant
   * @default
   */
   ...
   *  Parameters/configuration object for BlankTable
   *
   ...
   * Initialize module.
   *
   * @class H5P.BlankTable
   * @extends H5P.Question
   * @param {Params} params
   * @param {number} id Content identification
   * @param {Object} contentData Task specific content data
   */
  function BlankTable(params, id, contentData) {
    var self = this;

    // Inheritance
    Question.call(self, 'blankTable');
	...
	"Oslo is the capital of *Norway*."
      ...
      notFilledOut: "Please fill in all BlankTable to view solution",
  ...
  // Inheritance
  BlankTable.prototype = Object.create(Question.prototype);
  BlankTable.prototype.constructor = BlankTable;

  /**
   * Registers this question type's DOM elements before they are attached.
   * Called from H5P.Question.
   */
  BlankTable.prototype.registerDomElements = function () {
    var self = this;
    ...
    const labelId = 'h5p-BlankTable-instructions-' + BlankTable.idCounter;
    ...
    const lsData = this.getFromLocalStorage();
    if(lsData){
      this.previousState = lsData;
    }
   ...
   */
  BlankTable.prototype.registerButtons = function () {
    var self = this;
  ...
  /**
   * Find BlankTable in a string and run a handler on those BlankTable
   *
   * @param {string} question
   *   Question text containing BlankTable enclosed in asterisks.
   * @param {function} handler
   *   Replaces the BlankTable text with an input field.
   * @returns {string}
   *   The question with BlankTable replaced by the given handler.
   */
  BlankTable.prototype.handleBlanks = function (question, handler) {
	...
	while (clozeStart !== -1 && clozeEnd !== -1) {
		...
		if (clozeEnd === -1) {continue; }
		const clozeContent = question.substring(clozeStart, clozeEnd);
		let replacer = '';
  ...
  BlankTable.prototype.getLocalStorageID = function(){
    return "H5P_BlankTable_"+this.contentId;
  }

  BlankTable.prototype.saveToLocalStorage = function(){
     const lsID = this.getLocalStorageID();
     const lsContent = JSON.stringify(this.getCurrentState());
     localStorage.setItem(lsID,lsContent);
  }

  BlankTable.prototype.getFromLocalStorage = function(){
     const lsID = this.getLocalStorageID();
     const lsContent = localStorage.getItem(lsID);
     return lsContent ? JSON.parse(lsContent) : undefined;
  }

  BlankTable.prototype.queueLocalStorage = function(){
    const self = this;
    if(this.queueLocalStorageCB){clearTimeout(this.queueLocalStorageCB);}
    this.queueLocalStorageCB = setTimeout(function(){
      self.saveToLocalStorage();
      self.queueLocalStorageCB = undefined;
    },500);
  };

  BlankTable.prototype.handleBlankTable = function (question, handler) {
    const raw = this.handleBlanks(question,handler);
    const p = this.params;
    const tag = (x,y) => {
      if(p.headerColumn && (x == 0)){return 'th';}
      if(p.headerRow && (y == 0)){return 'th';}
      return 'td';
    }
	...
	if(trimmed === ""){continue;}
	const cols = trimmed.split(";").map((col,x) => `<${tag(x,y)}>${col}</${tag(x,y)}>`);
	y++;
	ret += `<tr>${cols.join("")}</tr>`;
    }
   ...
   */
  BlankTable.prototype.createQuestions = function (labelId) {
    var self = this;
    ...
    for (var i = 0; i < self.params.questions.length; i++) {
      ...
      question = self.handleBlankTable(question, function (solution) {
	// Create new cloze
	clozeNumber += 1;
	var defaultUserAnswer = (self.params.userAnswers.length > self.clozes.length ? self.params.userAnswers[self.clozes.length] : null);
	var cloze = new BlankTable.Cloze(solution, self.params.behaviour, defaultUserAnswer, {
	...
	afterCheck = function () {
	  ...
	  if (self.done || self.allBlankTableFilledOut()) {
   ...
   *
   */
  BlankTable.prototype.autoGrowTextField = function ($input) {
   ...
   */
  BlankTable.prototype.resetGrowTextField = function () {
    var self = this;
   ...
   */
  BlankTable.prototype.toggleButtonVisibility = function (state) {
   ...
   */
  BlankTable.prototype.allowSolution = function () {
    if (this.params.behaviour.showSolutionsRequiresInput === true) {
      if (!this.allBlankTableFilledOut()) {
  ...
  /**
   * Check if all BlankTable are filled out
   *
   * @method allBlankTableFilledOut
   * @return {boolean} Returns true if all BlankTable are filled out.
   */
  BlankTable.prototype.allBlankTableFilledOut = function () {
    return !this.clozes.some(function (cloze) {
      return cloze.isQuestion() && !cloze.filledOut();
    });
  };
   ...
   */
  BlankTable.prototype.markResults = function () {
    var self = this;
   ...
   */
  BlankTable.prototype.removeMarkedResults = function () {
   ...
   */
  BlankTable.prototype.showCorrectAnswers = function (alwaysShowSolution) {
   ...
   */
  BlankTable.prototype.toggleAllInputs = function (enabled) {
    for (var i = 0; i < this.clozes.length; i++) {
   ...
   */
  BlankTable.prototype.showSolutions = function () {
   ...
   * @public
   */
  BlankTable.prototype.resetTask = function () {
    this.answered = false;
    this.hideEvaluation();
    this.hideSolutions();
    this.clearAnswers();
    ...
    const lsData = this.getFromLocalStorage();
    if(lsData){this.previousState = lsData;}
   ...
   * @public
   */
  BlankTable.prototype.hideButtons = function () {
    this.toggleButtonVisibility(STATE_FINISHED);
  };
   ...
   */
  BlankTable.prototype.triggerAnswered = function () {
    this.answered = true;
   ...
   */
  BlankTable.prototype.getXAPIData = function () {
    var xAPIEvent = this.createXAPIEventTemplate('answered');
    this.addQuestionToXAPI(xAPIEvent);
    this.addResponseToXAPI(xAPIEvent);
    return {
   ...
   * @return {Object}
   */
  BlankTable.prototype.getxAPIDefinition = function () {
    var definition = {};
    ...
    for (var i = 0; i < this.params.questions.length; i++) {
      var question = this.handleBlankTable(this.params.questions[i], function (solution) {
	// Collect all solution combinations for the H5P Alternative extension
	clozeSolutions.push(solution.solutions);

	// Create a basic response pattern out of the first alternative for each BlankTable field
    ...
    // Set the basic response pattern (not supporting multiple alternatives for BlankTable)
   ...
   */
  BlankTable.prototype.addQuestionToXAPI = function (xAPIEvent) {
   ...
   */
  BlankTable.prototype.parseSolution = function (solutionText) {
    var tip, solution;
   ...
   */
  BlankTable.prototype.addResponseToXAPI = function (xAPIEvent) {
   ...
   */
  BlankTable.prototype.getxAPIResponse = function () {
    var usersAnswers = this.getCurrentState();
    return usersAnswers.join('[,]');
  };

  /**
   * Show evaluation widget, i.e: 'You got x of y BlankTable correct'
   */
  BlankTable.prototype.showEvaluation = function () {
   ...
   */
  BlankTable.prototype.hideEvaluation = function () {
   ...
   */
  BlankTable.prototype.hideSolutions = function () {
   ...
   */
  BlankTable.prototype.getMaxScore = function () {
    var self = this;
    let ret = 0;
    for (var i = 0; i < self.clozes.length; i++) {
      if(!self.clozes[i].isQuestion()){continue;}
      ret++;
    }
    return ret;
  };
   ...
   */
  BlankTable.prototype.getScore = function () {
    var self = this;
    var correct = 0;
    for (var i = 0; i < self.clozes.length; i++) {
      if(!self.clozes[i].isQuestion()){continue;}
  ...
  BlankTable.prototype.getTitle = function () {
   ...
   */
  BlankTable.prototype.clearAnswers = function () {
   ...
   * @returns {Boolean}
   */
  BlankTable.prototype.getAnswerGiven = function () {
   ...
   * @param {jQuery} $input
   */
  BlankTable.setFocus = function ($input) {
    setTimeout(function () {
      $input.focus();
    }, 1);
  };
   ...
   */
  BlankTable.prototype.getCurrentState = function () {
    var clozesContent = [];
   ...
   */
  BlankTable.prototype.setH5PUserState = function () {
    var self = this;
   ...
   */
  BlankTable.prototype.disableInput = function () {
    this.$questions.find('input').attr('disabled', true);
  };

  BlankTable.idCounter = 0;

  return BlankTable;
})(H5P.jQuery, H5P.Question);

/**
 * Static utility method for parsing H5P.BlankTable qestion into a format useful
 ...
 * @param {string} question
 */
H5P.BlankTable.parseText = function (question) {
  var blank = new H5P.BlankTable({ question: question });
